...
import static org.hibernate.boot.models.JpaAnnotations.NAMED_NATIVE_QUERY;
import static org.hibernate.boot.models.JpaAnnotations.NAMED_QUERY;
...
import static org.hibernate.boot.models.JpaAnnotations.SQL_RESULT_SET_MAPPING;
...
public class GlobalRegistrationsImpl implements GlobalRegistrations, GlobalRegistrar {
	 ...
	 */
	private String determineGeneratorRegistrationName(ClassDetails classDetails, String explicitName) {
		if ( StringHelper.isNotEmpty( explicitName ) ) {
			return explicitName;
		}

		final boolean isPackageInfo = classDetails.getName().endsWith( ".package-info" );
		if ( isPackageInfo ) {
			throw new AnnotationException( "Identifier-generators defined on package must specify a name - " + classDetails.getName() );
		}
		...
		if ( entityAnnotation != null ) {
			...
			if ( StringHelper.isNotEmpty( explicitEntityName ) ) {
	...
	private String determineImplicitGeneratorNameBase(ClassDetails classDetails) {
		final Entity entityAnnotation = classDetails.getDirectAnnotationUsage( ENTITY );
		if ( entityAnnotation != null ) {
			final String explicitEntityName = entityAnnotation.name();
	...
	public void collectSequenceGenerator(ClassDetails classDetails, SequenceGenerator usage) {
		final String registrationName = determineGeneratorRegistrationName( classDetails, usage.name() );
	...
	public void collectQueryReferences(ClassDetails classDetails) {
		collectNamedSqlResultSetMappings( classDetails );
		collectJpaNamedQueries( classDetails );
		collectJpaNamedNativeQueries( classDetails );
		collectStoredProcedureQueries( classDetails );
	}

	private void collectNamedSqlResultSetMappings(ClassDetails classDetails) {
		classDetails.forEachRepeatedAnnotationUsages(
				...
				(sqlResultSetMapping) -> {
					...
					sqlResultSetMappingRegistrations.put(
							...
							new SqlResultSetMappingRegistration( sqlResultSetMapping.name(), sqlResultSetMapping )
					);
				}
		);
	}

	private void collectJpaNamedQueries(ClassDetails classDetails) {
		classDetails.forEachRepeatedAnnotationUsages(
				NAMED_QUERY,
				sourceModelContext,
				(namedQuery) -> {
					...
					namedQueryRegistrations.put(
							namedQuery.name(),
							new NamedQueryRegistration( namedQuery.name(), namedQuery )
					);
				}
		);
	}

	private void collectJpaNamedNativeQueries(ClassDetails classDetails) {
		classDetails.forEachRepeatedAnnotationUsages(
				NAMED_NATIVE_QUERY,
				sourceModelContext,
				(namedQuery) -> {
					...
					namedNativeQueryRegistrations.put(
							namedQuery.name(),
							new NamedNativeQueryRegistration( namedQuery.name(), namedQuery )
					);
				}
		);
	}

	private void collectStoredProcedureQueries(ClassDetails classDetails) {
		classDetails.forEachRepeatedAnnotationUsages(
				...
				(namedQuery) -> {
					...
					namedStoredProcedureQueryRegistrations.put(
							namedQuery.name(),
							new NamedStoredProcedureQueryRegistration( namedQuery.name(), namedQuery )
					);
				}
		);
	}
	...
	private void collectNamedSqlResultSetMappings(
		...
		jaxbSqlResultSetMappings.forEach( (jaxbMapping) -> {
			final SqlResultSetMappingJpaAnnotation annotation = SQL_RESULT_SET_MAPPING.createUsage( sourceModelContext );
